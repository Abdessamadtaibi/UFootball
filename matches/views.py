...
def get_user_allowed_matches(user):
    ...
    # Viewer users: see all matches from public tournaments
...
class FinishMatchView(APIView):
    ...
    def post(self, request, match_id):
        ...
        try:
            from teams.season_stats import update_season_stats_for_match
            update_season_stats_for_match(match)
        except Exception:
            pass  # Don't block the match finish if stats update fails
...
class MatchEventsView(APIView):
    ...
    def post(self, request, match_id):
        ...
        # Auto-update player stats in MatchLineup
        if event.player:
            try:
                lineup = MatchLineup.objects.get(match=match, team_id=team_id, player=event.player)
                if event.event_type in ['goal', 'penalty_goal']:
                    lineup.goals_scored += 1
                elif event.event_type == 'yellow_card':
                    lineup.yellow_cards += 1
                ...
                pass  # Player not in lineup, skip stats update
