...
class MatchEvent(models.Model):
    ...
    # Manual player info when player is not in lineup
    player_name = models.CharField(max_length=100, blank=True, null=True, verbose_name="Nom du joueur")
    player_number = models.PositiveIntegerField(null=True, blank=True, verbose_name="Numéro du joueur")
    
    event_type = models.CharField(max_length=15, choices=EVENT_TYPE_CHOICES)
    minute = models.PositiveIntegerField(validators=[MinValueValidator(0), MaxValueValidator(120)])
    ...
    def __str__(self):
        ...
        # Handle nullable player
        name = self.player.full_name if self.player else (self.player_name or "Unknown")
        return f"{self.get_event_type_display()} - {name} ({time_str})"
